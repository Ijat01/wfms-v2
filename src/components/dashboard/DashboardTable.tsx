...
import { getAllTaskDueThisWeek, getAllTaskInprogress, getAllTaskComplete } from "@/lib/data";

export async function DashboardTable() {
  const inprogress = await getAllTaskInprogress();
  const complete = await getAllTaskComplete();
  const thisweek = await getAllTaskDueThisWeek();
